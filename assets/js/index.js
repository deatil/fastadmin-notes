require(['notes'], function (undefined) {
    ...
    $("body").on('click', '.js-notes-fast-note-item', function() {
        ...
        var tags = $(this).find('.notes-fast-note-item-content').data('tags');
        
        var content = '<textarea placeholder="'+__('Notes Content')+'" class="notes-fast-note js-notes-fast-note" data-noteid="'+note_id+'"></textarea>';
        if (tags) {
            content += '<div class="notes-note-tags">' + formatter.tags(tags) + '</div>';
        }
    ...
    $("body").on('click', '.js-notes-fast-list', function() {
        ...
        $(data).each(function(index, edata) {
            ...
            if (edata.tags != '') {
                edit_time += ' <i class="fa fa-tags" title="'+__('Note has tags')+'"></i>';
            }
            ...
            html += '<li class="notes-fast-note-item js-notes-fast-note-item notes-fast-note-item-' + edata.id + '" data-id="' + edata.id + '">'
                + '<div class="notes-fast-note-item-date">'
                    ...
                    + '</a>'
                + '</div>'
                + '<div class="notes-fast-note-item-content" data-tags="' + edata.tags + '">'
                + content
                + '</div>'
            + '</li>';
        });
            ...
            Fast.api.ajax({
                ...
                if (ret.hasOwnProperty("code")) {
                    ...
                    if (ret.code == 1) {
                        ...
                        Fast.api.success(msg ? msg : __('Fast remove note success'));
                    } else {
                        Fast.api.error(msg ? msg : __('Fast remove note error'));
                    }
                } else {
